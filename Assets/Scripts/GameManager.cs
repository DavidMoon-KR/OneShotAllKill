...
class Objects
{
    public List<GameObject> HintClearTargets;
}
...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] private List<Objects> m_HintClearTargets;   // 힌트를 통해서 처리해야 할 타겟들
    ...
    public bool IsShowHintMessage { set => m_IsShowHintMessage = value; }
    public int NowHintCount { get  => m_NowHintCount; set => m_NowHintCount = value; }
    ...
    void Start()
    {
        ...
        if (m_AllHintCount >= 1)
        {
            TimerCheckDontDestroy.Instance.SetTimerOptions();
            if (m_NowHintCount < m_AllHintCount)
            TimerCheckDontDestroy.Instance.Setup(m_HintShowDelay[m_NowHintCount]);
        }        
    }
    ...
    void Update()
    {
        ...
        if (!m_IsActiveHint && m_NowHintCount > 0 && m_IsApplyHint[m_NowHintCount - 1] || Input.GetKeyDown(KeyCode.F) && m_IsShowHintMessage)
        {
            // 힌트 메시지 끄기
        ...
        if (m_IsActiveHint && m_HintWallCount >= m_AllHintWallCount[m_NowHintCount - 1])
        {
            // 만약 위치하는 것 이외의 조건으로 처리해야할 오브젝트가 아직 남아있다면 힌트 활성화 상태 유지
            if (m_HintClearTargets[m_NowHintCount-1].HintClearTargets != null && !ClearHintTargetCheck())
            {
                return;
            }
            ...
            TimerCheckDontDestroy.Instance.IsHintActive = false;
            if (m_NowHintCount < m_AllHintCount)
            {
                TimerCheckDontDestroy.Instance.Setup(m_HintShowDelay[m_NowHintCount]);
            }
    ...
    {
        foreach (GameObject obj in m_HintClearTargets[m_NowHintCount-1].HintClearTargets)
        {
    ...
    // 힌트
    public void HintTimer(int p_count)
    {
        UIManager.Instance.ShowHint();
        m_IsShowHintMessage = true;
        m_NowHintCount = p_count;
    }
}
