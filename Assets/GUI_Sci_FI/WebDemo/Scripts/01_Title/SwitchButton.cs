...
public class SwitchButton : MonoBehaviour, IPointerUpHandler, IPointerDownHandler, IDragHandler {
    ...
    void Start()
    {
        _isSwitchOn = GameDataManager.Instance.Data.CameraShakeEnabled;

        if (_isSwitchOn)
        {
            textOn.color = onColor;
            textOff.color = offColor;
			imageHandle.transform.localPosition = new Vector3(onPos.localPosition.x, 0f, 0f);
        }
        else
        {
            textOn.color = offColor;
            textOff.color = onColor;
            imageHandle.transform.localPosition = new Vector3(offPos.localPosition.x, 0f, 0f);
        }
    }
	...
	public void SwitchCheck () {
		...
		GameDataManager.Instance.Data.CameraShakeEnabled = _isSwitchOn;
