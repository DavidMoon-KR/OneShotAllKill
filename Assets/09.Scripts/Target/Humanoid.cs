...
public class Humanoid : MonoBehaviour
{
    ...
    [SerializeField] readonly float m_AgroTime = 600;
    ...
    private bool m_WaitForMove = false;
        ...
        if (m_IsHit == false)//총알에 맞지 않았을 경우
        {
            ...
            else
            {
                ...
                //폭파가 감지 되었다면
                {
                    ...
                    float num = Vector3.Distance(transform.position, m_ExplosionedPos);
                    m_Path = new NavMeshPath();
                    m_HasPath = m_Agent.CalculatePath(m_ExplosionedPos, m_Path);

                    if (m_HasPath && m_Path.status != NavMeshPathStatus.PathComplete)
                    {
