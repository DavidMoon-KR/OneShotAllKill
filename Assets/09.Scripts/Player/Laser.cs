...
public class Laser : MonoBehaviour
{
    ...
    private int m_Reflections = 20;//튕기는 횟수 ※이거를 수정해야 여러번 튕깁니다※
    ...
    private List<GameObject> m_PassableWalls = new List<GameObject>();
    ...
    bool PassableCheck(GameObject Wall)
    {
        for (int i = 0; i < m_PassableWalls.Count; i++)
        {
            for(int j = 0;j< m_PassableWalls[i].transform.childCount;j++)
            {
                if (m_PassableWalls[i].transform.GetChild(j).gameObject == Wall.gameObject)//여기서 이제 하나씩 비교 후 true or false 리턴
                {
                    return true;
                }
            }            
        ...
        for (int i = 0; i < m_Reflections; i++)
        {
            ...
            {                
                ...
                {    
                    ...
                    {
                        for (int j = 0; j < m_CollidedObject.transform.GetComponent<EnergyWallButton>().m_Shield.Count; j++)
                        {
                            m_PassableWalls.Add(m_CollidedObject.transform.GetComponent<EnergyWallButton>().m_Shield[j]);
                        }
                    }
                ...
                else if(m_CollidedObject.collider.tag == "Broken" || (m_CollidedObject.collider.tag == "EnergyWall") && !PassableCheck(m_CollidedObject.collider.gameObject))//총알이 부딛혔을때 파괴되는 오브젝트를 만났을때                                    
                {
